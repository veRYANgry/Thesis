...
public class NEATGATrainingManager {
       ...
       // options.setArgs("-ls " + "resources/test.lvl" );
		...
		while (true) {
			...
			if(first){
				...
				firstbest = (((NEATGeneticAlgorithmMario) this.ga).genBest());
				first = false;
			}
			...
			if ((((NEATGeneticAlgorithmMario) this.ga).genBest() > firstbest)){
				System.out.println("Improvment" + firstbest + "to -->" + ((NEATGeneticAlgorithmMario) this.ga).genBest());
		        options.setArgs("-ls " + (new Random(System.currentTimeMillis())).nextInt());
				//this.saveGA("ga");
				//this.loadGA("ga");
				//this.ga.savePopulationState("pop");
				//((NEATGeneticAlgorithmMario) this.ga).saveSpeciesState("spec");
				//((NEATGeneticAlgorithmMario) this.ga).loadSpeciesState("spec");
				//((NEATGeneticAlgorithmMario) this.ga).loadPopulationState("pop");
	...
	public void loadGA(String fileName) {
			...
			try {
				if (fileName != null) {
					System.out.println("loading Ga " + fileName);
	...
	public void saveGA(String fileName) {
		...
		try {
			if (fileName != null) {
				System.out.println("Saving GA " + fileName);
		...
		System.out.println("Saving GA...Done");
	}
