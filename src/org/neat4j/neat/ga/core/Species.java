		...
		while (i < this.specieList.size()) {
			...
			if (specie.isExtinct() || specie.specieMembers().size() == 0) {
				if (!specie.containsMember(champion) && (this.specieList.size() > 1)) {
					System.out.print("Removing specie " + specie.id() + " size:" + specie.specieMembers().size() + ":fage:" + specie.getCurrentFitnessAge() + ":age:" + ((NEATSpecie)specie).specieAge() + ":avF:" + specie.averageFitness());
					this.specieList.remove(i);
				} else {
					System.out.print("Specie " + specie.id() + " saved:" + specie.specieMembers().size() + ":fage:" + specie.getCurrentFitnessAge() + ":age:" + ((NEATSpecie)specie).specieAge() + ":avF:" + specie.averageFitness());
					specie.reprieve();
					i++;
				}
			} else {
