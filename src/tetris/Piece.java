 ...
 * piece is defined by the blocks that make up its body. See the
...
public class Piece {
	/*
	 * Implementation notes: -The starter code specs out a few simple things,
	 * but leaves the key algorithms for you. -Store the body as a Point[] array
	 * -The ivars in the Point class are .x and .y -Do not assume there are 4
	 * points in the body -- use array.length to keep the code general
	 */
	...
	/**
	 * Defines a new piece given the Points that make up its body. Makes its own
	 * copy of the array and the Points inside it. Does not set up the
	 * rotations.
	 * 
	 * This constructor is PRIVATE -- if a client wants a piece object, they
	 * must use Piece.getPieces().
	 */
	 ...
	 * piece, the skirt gives the lowest y value in the body. This useful for
	 ...
	 * 
	 * <p>
	 * Implementation: The Piece class pre-computes all the rotations once. This
	 * method just hops from one pre-computed rotation to the next in constant
	 * time.
	 */
	 ...
	 * tetris pieces. The next (counterclockwise) rotation can be obtained from
	 * each piece with the {@link #nextRotation()} message. In this way, the
	 * client can iterate through all the rotations until eventually getting
	 * back to the first rotation. (provided code)
	 */
	...
	protected static Point[] parsePoints(String string) {
		...
		try {
			...
			throw new RuntimeException("Could not parse x,y string:" + string); // cheap
																				// way
																				// to
																				// do
																				// assert
	...
	/**
	 * Given the "first" rotation of a piece piece, computes all the other
	 * rotations and links them all together by their next pointers. Returns the
	 * first piece. {@link nextRotation()} relies on the next pointers to get
	 * from one rotation to the next. Internally, uses Piece.equals() to detect
